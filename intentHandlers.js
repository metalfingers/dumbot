...
function validateResponse(user, conversationName, message) {
	...
	if (user.session.questionAsked) {
		...
		let convoComponent = conversations[conversationName]
													.components
...
function getNextComponent(user, conversationName, completionMessage) {
	...
	conversations[conversationName].components.forEach((component, i) => {
		...
		if (user.prefs[component.name] === undefined) {
			...
			if (user.session.conversations.some(i => i === conversationName) === false) {
...
const intentHandlers = {
	greeting: function(user, message) {
